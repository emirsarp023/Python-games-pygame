...
# oyun sonu yazıları
win = font2.render("YOU WIN !!!",True,(0,255,0))    # yeşil renkte you win yazısı
lose = font2.render("YOU LOSE !!!",True,(255,0,0))  # kırmızı renkte you lose yazısı
...
music_playing = True # müzik çalıyor 
lostsayisi= 5 # kaç uzaylı kaçarsa oyun lose olur
winsayisi = 20 # kazanmak için vurmak gereken uzaylı sayısı
...
while run:
        ...
        if e.type == QUIT:
           run = False
            ...
            # yeniden başlatma tuşu
                ...
                score = 0
                lost = 0 
                #ekrandaki mermileri temizledik
                for b in bullets: 
                    b.kill()
                #ekrandaki uzaylıları temizledik
                for m in monsters:
                    m.kill()
                for i in range(1,6):
                    monster = Enemy(img_enemy,randint(80,win_width-80),-40,80,50,randint(1,5)) # tek nesne
    ...
    if not finish:
        ...
        # score
        text = font1.render("Puan: " +str(score),1,(0,255,255)) # kırmızı renkli score yazısı
        window.blit(text,(10,20)) # sol ust köşe
        # lost
        text_lose = font1.render("Kayıp: " +str(lost),1,(0,255,255)) # kırmızı renkli score yazısı
        ...
        for c in collides:
            ...
            monsters.add(monster) # nesnelerin hepsini gruba attık
        # kaybetme durumu
        if sprite.spritecollide(ship,monsters,False) or lost >= lostsayisi:
            finish = True 
